...
function SWV2_Spawn_SENT( player, EntityName, tr )
	...
	if ( sent ) then
		...
		local SpawnableEntities = list.Get( "SWVehiclesEU" )
...
concommand.Add( "swv2_spawnsent", function( ply, cmd, args ) SWV_Spawn_SENT( ply, args[1] ) end ) 

function SWV2_CCGiveSWEP( player, command, arguments )

	if ( arguments[1] == nil ) then return end

	-- Make sure this is a SWEP
	local swept = list.Get( "SWVehiclesEU.Weapons" );
	local swep;
...
concommand.Add( "swv2_giveswep", SWV2_CCGiveSWEP )

--[[---------------------------------------------------------
	-- Give a swep.. duh.
-----------------------------------------------------------]]
function SWV2_Spawn_Weapon( Player, wepname, tr )
...
concommand.Add( "swv2_spawnswep", function( ply, cmd, args ) SWV2_Spawn_Weapon( ply, args[1] ) end )    

end

if(CLIENT) then
spawnmenu.AddCreationTab( "Star Wars Vehicles: EU Pack", function()
	local ctrl = vgui.Create( "SpawnmenuContentPanel" )
    ctrl:CallPopulateHook( "SWVehiclesEUTab" );
    return ctrl;
end, "icons16/other.png", 60 )

spawnmenu.AddContentType( "swvehicleeu", function( container, obj )
    ...
    icon.DoClick = function()
        RunConsoleCommand( "swv2_spawnsent", obj.spawnname );
...
spawnmenu.AddContentType( "swweapon2", function( container, obj )
	...
	icon.DoClick = function()
		...
		RunConsoleCommand( "swv2_giveswep", obj.spawnname )
		surface.PlaySound( "ui/buttonclickrelease.wav" )
	...
	icon.DoMiddleClick = function()
		...
		RunConsoleCommand( "swv2_spawnswep", obj.spawnname )
		surface.PlaySound( "ui/buttonclickrelease.wav" )
...
hook.Add( "SWVehiclesEUTab", "AddEntityContent", function( pnlContent, tree, node )
    ...
    SpawnableEntities = list.Get( "SWVehiclesEU.Weapons" )
	if ( SpawnableEntities ) then
		for k, v in pairs( SpawnableEntities ) do

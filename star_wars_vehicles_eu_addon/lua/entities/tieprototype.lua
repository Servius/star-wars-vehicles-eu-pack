...
ENT.PrintName = "Advanced Tie Prototype";
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("tieprototype"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 150 + self:GetRight() * 25 + self:GetUp() * 80,
        Left = self:GetPos() + self:GetForward() * 150 + self:GetRight() * -25 + self:GetUp() * 80,
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-105+self:GetUp()*140+self:GetRight()*-0,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("TiePrototype", NULL)
    ...
    hook.Add("CalcView", "TiePrototypeview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function TiePrototypeReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingTiePrototype");
		local self = p:GetNWEntity("TiePrototype");
    ...
    hook.Add("HUDPaint", "TiePrototypeReticle", TiePrototypeReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

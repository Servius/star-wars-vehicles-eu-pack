...
ENT.EntModel = "models/bsp_bayonet/bsp_bayonet.mdl"
ENT.Vehicle = "bsp_bayonet"
ENT.StartHealth = 17000;
ENT.DontLock = true;
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("bsp_bayonet");
...
function ENT:Initialize()
	...
	self.HasWings = false;
	self.CanShoot = false;
	self.Bullet = CreateBulletStructure(75,"green",false);
	self.FireDelay = 0.25;
	self.HasLightspeed = true;
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*-270+self:GetForward()*-50+self:GetRight()*250, self:GetAngles()+Angle(0,150,-25)},
		{self:GetPos()+self:GetUp()*-270+self:GetForward()*-50+self:GetRight()*-250, self:GetAngles()+Angle(0,30,-25)},
	}
	self.GunnerSeats = {};
	...
	self.LeftWeaponLocations = {
		self:GetPos()+self:GetUp()*-270+self:GetForward()*200+self:GetRight()*450,
		self:GetPos()+self:GetUp()*-270+self:GetForward()*230+self:GetRight()*450,
		
		self:GetPos()+self:GetUp()*-270+self:GetForward()*120+self:GetRight()*450,
		self:GetPos()+self:GetUp()*-270+self:GetForward()*80+self:GetRight()*420,
		
		self:GetPos()+self:GetUp()*-270+self:GetForward()*-100+self:GetRight()*380,
		self:GetPos()+self:GetUp()*-270+self:GetForward()*-60+self:GetRight()*350,	
	}
...
hook.Add("PlayerLeaveVehicle", "BayonetSeatExit", function(p,v)
...
function ENT:FireLeft(angPos)
	...
	if(self.NextUse.Fire < CurTime()) then
		...
		self:EmitSound(self.FireSound,100,math.random(70,100));
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*1422+self:GetUp()*180+self:GetRight()*420,
				self:GetPos()+self:GetForward()*1422+self:GetUp()*180+self:GetRight()*-420,
				
				self:GetPos()+self:GetForward()*1370+self:GetUp()*-80+self:GetRight()*350,
				self:GetPos()+self:GetForward()*1370+self:GetUp()*-80+self:GetRight()*470,
				self:GetPos()+self:GetForward()*1370+self:GetUp()*-80+self:GetRight()*590,
	...
	function ENT:Effects()
		...
		for k,v in pairs(self.EnginePos) do
			...
			heatwv:SetDieTime(0.15);
			heatwv:SetStartAlpha(255);
			heatwv:SetEndAlpha(255);
			heatwv:SetStartSize(155);
			heatwv:SetEndSize(85);
	...
	function BayonetReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingBayonet");
		local self = p:GetNWEntity("Bayonet");
		...
		if(Flying and IsValid(self)) then
			...
			local x = ScrW()/10;
			local y = ScrH()/4*3.5;
			SW_HUD_DrawHull(17000,x,y);		
			
		elseif(LeftGunner and IsValid(self)) then

			local WeaponsPos = {
				self:GetPos()+self:GetUp()*160+self:GetForward()*30+self:GetRight()*400,
				self:GetPos()+self:GetUp()*160+self:GetForward()*0+self:GetRight()*380,
		
				self:GetPos()+self:GetUp()*160+self:GetForward()*-500+self:GetRight()*260,
				self:GetPos()+self:GetUp()*160+self:GetForward()*-530+self:GetRight()*240,
		
				self:GetPos()+self:GetUp()*155+self:GetForward()*-1100+self:GetRight()*180,
				self:GetPos()+self:GetUp()*160+self:GetForward()*-1130+self:GetRight()*150,		
			}
	...
	hook.Add("HUDPaint", "BayonetReticle", BayonetReticle)

...
ENT.EntModel = "models/sfp_ybwing/sfp_ybwing.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_ybwing" --The internal name for the ship. It cannot be the same as a different ship.
ENT.StartHealth = 2300; --How much health they should have.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_ybwing"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.Bullet = CreateBulletStructure(40,"red",true); -- The first number is bullet damage, the second colour. green and red are the only options. (Set to blue for ion shot, the damage will be halved but ships will be disabled after consecutive hits). The final one is for splash damage. Set to true if you don't want splashdamage.
...
function ENT:Effects()
    ...
    hook.Add("CalcView", "sfp_ybwingview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_ybwingReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_ybwing");
		local self = p:GetNWEntity("sfp_ybwing");
		if(Flying and IsValid(self)) then
			SW_HUD_DrawHull(2300); -- Replace 1000 with the starthealth at the top
    ...
    hook.Add("HUDPaint", "sfp_ybwingReticle", sfp_ybwingReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

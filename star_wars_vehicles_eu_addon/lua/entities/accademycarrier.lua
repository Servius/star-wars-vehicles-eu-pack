...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("accademycarrier"); -- This should be the same name as the file
...
function ENT:Initialize()
	...
	self.SeatPos = {
		...
		{self:GetPos()+self:GetForward()*-100+self:GetUp()*0+self:GetRight()*50,self:GetAngles(),Vector(-500,100,0)},
		{self:GetPos()+self:GetForward()*-100+self:GetUp()*0+self:GetRight()*100,self:GetAngles(),Vector(-500,100,0)},
...
function ENT:Effects()
    ...
    local function CalcView()      
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("accademycarrier", NULL)
    ...
    hook.Add("CalcView", "accademycarrierview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function accademycarrierReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingaccademycarrier");
		local self = p:GetNWEntity("accademycarrier");
    ...
    hook.Add("HUDPaint", "accademycarrierReticle", accademycarrierReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

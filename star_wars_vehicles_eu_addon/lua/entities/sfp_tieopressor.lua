...
ENT.EntModel = "models/sfp_tieopressor/sfp_tieopressor.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tieopressor" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tieopressor"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 230 + self:GetRight() * 25 + self:GetUp() * 50,
        Left = self:GetPos() + self:GetForward() * 230 + self:GetRight() * -25 + self:GetUp() * 50,
    }
	...
	self.ExitModifier = {x=0,y=-200,z=10}
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		local pos;
		if(fire == 1) then
			pos = self:GetPos()+self:GetUp()*50+self:GetForward()*100+self:GetRight()*-20;
			self.NextUse.Torpedos = CurTime()-0.1;
		elseif(fire == 2) then
			pos = self:GetPos()+self:GetUp()*50+self:GetForward()*100+self:GetRight()*20;
			self.NextUse.Torpedos = CurTime()+0.25;
		elseif(fire == 3) then
			pos = self:GetPos()+self:GetUp()*62+self:GetForward()*100+self:GetRight()*-15;
			self.NextUse.Torpedos = CurTime()-0.1;
		elseif(fire == 4) then
			pos = self:GetPos()+self:GetUp()*62+self:GetForward()*100+self:GetRight()*15;
		...
		self:FireTorpedo(pos,e,1500,500,Color(120,180,255,200),10);
		fire = fire + 1;
		if(fire > 4) then
			fire = 1;
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*20+self:GetUp()*68
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetColor(120,160,255) -- Set the colour in RGB. This is more of an overlay colour effect and doesn't change the material source.
		...
		dynlight.R = 120; -- Colour R
		dynlight.G = 160; -- Colour G
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tieopressor", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,500,300,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_tieopressorview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tieopressorReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tieopressor");
		local self = p:GetNWEntity("sfp_tieopressor");
    ...
    hook.Add("HUDPaint", "sfp_tieopressorReticle", sfp_tieopressorReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

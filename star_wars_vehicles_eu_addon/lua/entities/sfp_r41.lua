...
ENT.EntModel = "models/sfp_r41/sfp_r41.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_r41" --The internal name for the ship. It cannot be the same as a different ship.
ENT.StartHealth = 1400; --How much health they should have.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_r41"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.Bullet = CreateBulletStructure(70,"red",true); -- The first number is bullet damage, the second colour. green and red are the only options. (Set to blue for ion shot, the damage will be halved but ships will be disabled after consecutive hits). The final one is for splash damage. Set to true if you don't want splashdamage.
...
function ENT:Effects()
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_r41", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,450,150,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_r41view", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_r41Reticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_r41");
		local self = p:GetNWEntity("sfp_r41");
		if(Flying and IsValid(self)) then
			SW_HUD_DrawHull(1400); -- Replace 1000 with the starthealth at the top
    ...
    hook.Add("HUDPaint", "sfp_r41Reticle", sfp_r41Reticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

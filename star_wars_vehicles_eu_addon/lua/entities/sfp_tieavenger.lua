...
ENT.EntModel = "models/sfp_tieavenger/sfp_tieavenger.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tieavenger" --The internal name for the ship. It cannot be the same as a different ship.
ENT.StartHealth = 2200; --How much health they should have.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tieavenger"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        FarRight = self:GetPos() + self:GetForward() * 210 + self:GetRight() * 110 + self:GetUp() * 50,
        FarLeft = self:GetPos() + self:GetForward() * 210 + self:GetRight() * -110 + self:GetUp() * 50,
        Right = self:GetPos() + self:GetForward() * 210 + self:GetRight() * 110 + self:GetUp() * 120,
        Left = self:GetPos() + self:GetForward() * 210 + self:GetRight() * -110 + self:GetUp() * 120,
    }
	...
	self.FireGroup = {"Left","FarLeft","Right","FarRight"} -- In this example, the weapon positions set above will fire with Left and TopLeft at the same time. And Right and TopRight at the same time.
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		...
		self:FireTorpedo(pos,e,1500,500,Color(140,140,255,200),10);
		fire = fire + 1;
		if(fire > 4) then
			fire = 1;
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-150+self:GetUp()*95+self:GetRight()*-35,
		self:GetPos()+self:GetForward()*-150+self:GetUp()*95+self:GetRight()*35,
		self:GetPos()+self:GetForward()*-150+self:GetUp()*85+self:GetRight()*-35,
		self:GetPos()+self:GetForward()*-150+self:GetUp()*85+self:GetRight()*35,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		dynlight.Pos = v; -- Position from the table
 		dynlight.Brightness = 2; -- Brightness, Don't go above 10. It's blinding
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tieavenger", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,530,200,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_tieavengerview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tieavengerReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tieavenger");
		local self = p:GetNWEntity("sfp_tieavenger");
		if(Flying and IsValid(self)) then
			SW_HUD_DrawHull(2200); -- Replace 1000 with the starthealth at the top
    ...
    hook.Add("HUDPaint", "sfp_tieavengerReticle", sfp_tieavengerReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.EntModel = "models/sfp_xfeo/sfp_xfeo.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_xfeo" --The internal name for the ship. It cannot be the same as a different ship.
ENT.StartHealth = 1800; --How much health they should have.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_xfeo"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        TopRight = self:GetPos() + self:GetForward() * 145 + self:GetRight() * 95 + self:GetUp() * 220,
        TopLeft = self:GetPos() + self:GetForward() * 145 + self:GetRight() * -95 + self:GetUp() * 220,
        Right = self:GetPos() + self:GetForward() * 145 + self:GetRight() * 95 + self:GetUp() * 120,
        Left = self:GetPos() + self:GetForward() * 145 + self:GetRight() * -95 + self:GetUp() * 120,
    }
	...
	self.ExitModifier = {x=125,y=150,z=10}
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		local pos;
		if(fire == 1) then
			pos = self:GetPos()+self:GetUp()*170+self:GetForward()*100+self:GetRight()*-35;
		...
		self:FireTorpedo(pos,e,1500,500,Color(255,2,20,230),10);
		fire = fire + 1;
		if(fire > 2) then
			fire = 1;
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-247+self:GetUp()*137+self:GetRight()*-58,
		self:GetPos()+self:GetForward()*-247+self:GetUp()*137+self:GetRight()*60,
		self:GetPos()+self:GetForward()*-247+self:GetUp()*214+self:GetRight()*-58,
		self:GetPos()+self:GetForward()*-247+self:GetUp()*214+self:GetRight()*60,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetStartSize(12) -- Start size. Just play around to find the right size.
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_xfeo", NULL)
    ...
    hook.Add("CalcView", "sfp_xfeoview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_xfeoReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_xfeo");
		local self = p:GetNWEntity("sfp_xfeo");
		if(Flying and IsValid(self)) then
			SW_HUD_DrawHull(1800); -- Replace 1000 with the starthealth at the top
    ...
    hook.Add("HUDPaint", "sfp_xfeoReticle", sfp_xfeoReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

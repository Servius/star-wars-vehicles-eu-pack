...
ENT.EntModel = "models/fp_cc7700/fp_cc7700.mdl"
ENT.Vehicle = "CC 7700 Corvette"
...
function ENT:Initialize()
	...
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*0+self:GetForward()*0+self:GetRight()*200, self:GetAngles()+Angle(0,180,0)},
		{self:GetPos()+self:GetUp()*0+self:GetForward()*0+self:GetRight()*-200, self:GetAngles()},
	}
	self.GunnerSeats = {};
...
hook.Add("PlayerLeaveVehicle", "cc7700SeatExit", function(p,v)
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*580+self:GetUp()*-0+self:GetRight()*115,
				self:GetPos()+self:GetForward()*580+self:GetUp()*-0+self:GetRight()*-115,
				self:GetPos()+self:GetForward()*580+self:GetUp()*-0
			}
			self:Effects();
		end
	end	
	...
	function cc7700Reticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingcc7700");
		local self = p:GetNWEntity("cc7700");
	...
	hook.Add("HUDPaint", "cc7700Reticle", cc7700Reticle)

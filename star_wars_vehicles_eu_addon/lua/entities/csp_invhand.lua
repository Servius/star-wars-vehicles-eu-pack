...
ENT.EntModel = "models/csp_invhand/csp_invhand.mdl"
ENT.Vehicle = "InvHand"
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("csp_invhand");
...
function ENT:Initialize()
	...
	self.HasWings = false;
	self.CanShoot = false;
	self.Bullet = CreateBulletStructure(100,"red",false);
	self.FireDelay = 0.50;
	self.HasLightspeed = true;
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*350+self:GetForward()*-1500+self:GetRight()*500, self:GetAngles()+Angle(0,180,0)},
		{self:GetPos()+self:GetUp()*350+self:GetForward()*-1500+self:GetRight()*-500, self:GetAngles()},
	}
	self.GunnerSeats = {};
	...
	self.LeftWeaponLocations = {
		self:GetPos()+self:GetUp()*275+self:GetForward()*-2350+self:GetRight()*760,
		self:GetPos()+self:GetUp()*275+self:GetForward()*-2400+self:GetRight()*760,
		self:GetPos()+self:GetUp()*275+self:GetForward()*-2450+self:GetRight()*750,
		
		self:GetPos()+self:GetUp()*250+self:GetForward()*-1495+self:GetRight()*750,
		self:GetPos()+self:GetUp()*250+self:GetForward()*-1555+self:GetRight()*750,
		self:GetPos()+self:GetUp()*250+self:GetForward()*-1615+self:GetRight()*750,
		
		
		self:GetPos()+self:GetUp()*200+self:GetForward()*-800+self:GetRight()*650,
		self:GetPos()+self:GetUp()*200+self:GetForward()*-850+self:GetRight()*650,
		self:GetPos()+self:GetUp()*200+self:GetForward()*-900+self:GetRight()*650,
	}
	...
	self.ExitModifier = {x=1000,y=525,z=0};
...
function ENT:FireLeft(angPos)
	...
	if(self.NextUse.Fire < CurTime()) then
		...
		self:EmitSound(self.FireSound,100,math.random(60,100));
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*3600+self:GetUp()*-20+self:GetRight()*330,
				self:GetPos()+self:GetForward()*3600+self:GetUp()*-20+self:GetRight()*-330,
			}
			self.EnginePos3 = {
				self:GetPos()+self:GetForward()*3675+self:GetUp()*55
			}
			self.EnginePos2 = {
				self:GetPos()+self:GetForward()*3580+self:GetUp()*-150+self:GetRight()*180,
				self:GetPos()+self:GetForward()*3580+self:GetUp()*-150+self:GetRight()*-180,
				self:GetPos()+self:GetForward()*2925+self:GetUp()*-220+self:GetRight()*180,
				self:GetPos()+self:GetForward()*2925+self:GetUp()*-220+self:GetRight()*-180,
			}
			self:Effects();
		end
	...
	function ENT:Effects()
		...
		for k,v in pairs(self.EnginePos3) do
	...
	local function InvHandReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingInvHand");
		local self = p:GetNWEntity("InvHand");
	...
	hook.Add("HUDPaint", "InvHandReticle", InvHandReticle)

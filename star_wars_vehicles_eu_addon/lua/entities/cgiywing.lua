...
ENT.EntModel = "models/ywing/rep_ywinggold_servius.mdl"
ENT.FlyModel = "models/ywing/rep_ywinggold_servius.mdl"
ENT.Vehicle = "YWing"
ENT.StartHealth = 1500;
ENT.Allegiance = "Republic";
list.Set("SWVehicles", ENT.PrintName, ENT);
util.PrecacheModel("models/ywing/rep_ywinggold_servius.mdl")
...
function ENT:Initialize()
	...
	self.WeaponLocations = {
		Left = self:GetPos()+self:GetForward()*355+self:GetUp()*35+self:GetRight()*-30,
		Right = self:GetPos()+self:GetForward()*355+self:GetUp()*35+self:GetRight()*30,
	}
	self.WeaponsTable = {};
	//self:SpawnWeapons();
	...
	self.Bullet = CreateBulletStructure(85,"blue");
	...
	if(self.Inflight) then
		if(IsValid(self.Pilot)) then
			...
			if(IsValid(self.Pilot) and self.Pilot:KeyDown(IN_ATTACK2)) then
				self:FireBlast(self:GetPos()+self:GetForward()*500,true,0.6,600,false,20);
			end
		end
	end
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetUp()*40+self:GetRight()*140+self:GetForward()*-430,
				self:GetPos()+self:GetUp()*40+self:GetRight()*-150+self:GetForward()*-430,
			}

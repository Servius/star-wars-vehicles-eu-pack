...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("w_wing"); // This should be the same name as the file
...
function ENT:Effects()
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("w_wing", NULL)
    ...
    hook.Add("CalcView", "w_wingview", CalcView) // This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function w_wingReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingw_wing");
		local self = p:GetNWEntity("w_wing");
    ...
    hook.Add("HUDPaint", "w_wingReticle", w_wingReticle) // Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

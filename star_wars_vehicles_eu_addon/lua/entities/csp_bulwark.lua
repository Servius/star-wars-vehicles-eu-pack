...
ENT.EntModel = "models/csp_bulwark/csp_bulwark.mdl"
ENT.Vehicle = "csp_bulwark"
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("csp_bulwark");
...
function ENT:Initialize()
	...
	self.WeaponLocations = {
		Left = self:GetPos()+self:GetForward()*1000+self:GetUp()*250+self:GetRight()*-270,
		Right = self:GetPos()+self:GetForward()*1000+self:GetUp()*250+self:GetRight()*270,
	}
	self.WeaponsTable = {};
	...
	self.FireDelay = 0.35;
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*50+self:GetForward()*-600+self:GetRight()*500, self:GetAngles()+Angle(0,160,0)},
		{self:GetPos()+self:GetUp()*50+self:GetForward()*-600+self:GetRight()*-500, self:GetAngles()},
	}
	self.GunnerSeats = {};
	...
	self.LeftWeaponLocations = {
		self:GetPos()+self:GetUp()*50+self:GetForward()*-10+self:GetRight()*480,
		self:GetPos()+self:GetUp()*50+self:GetForward()*50+self:GetRight()*480,
		
		self:GetPos()+self:GetUp()*50+self:GetForward()*-290+self:GetRight()*480,
		self:GetPos()+self:GetUp()*50+self:GetForward()*-340+self:GetRight()*480,
		
		self:GetPos()+self:GetUp()*50+self:GetForward()*-630+self:GetRight()*440,
		self:GetPos()+self:GetUp()*50+self:GetForward()*-690+self:GetRight()*440,
		
		self:GetPos()+self:GetUp()*50+self:GetForward()*-980+self:GetRight()*380,
		self:GetPos()+self:GetUp()*50+self:GetForward()*-1040+self:GetRight()*380,
		
		self:GetPos()+self:GetUp()*50+self:GetForward()*-1340+self:GetRight()*320,
		self:GetPos()+self:GetUp()*50+self:GetForward()*-1390+self:GetRight()*320,	
	}
...
hook.Add("PlayerLeaveVehicle", "BulwarkSeatExit", function(p,v)
	if(IsValid(p) and IsValid(v)) then
		if(v.IsGunnerSeat) then
			local e = v:GetParent();
			if(v.IsRight) then
				e:GunnerExit(true,p);
			else
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*1150+self:GetUp()*-80+self:GetRight()*215,
				self:GetPos()+self:GetForward()*1150+self:GetUp()*-80+self:GetRight()*-215,
			}
			self.EnginePos2 = {
				self:GetPos()+self:GetForward()*1080+self:GetUp()*-165+self:GetRight()*55,
				self:GetPos()+self:GetForward()*1080+self:GetUp()*-165+self:GetRight()*-55,
				self:GetPos()+self:GetForward()*1080+self:GetUp()*-90
			}
			self:Effects();
		end
	end	
	...
	function ENT:Effects()
		...
		for k,v in pairs(self.EnginePos) do
			...
			heatwv:SetStartSize(145);
			heatwv:SetEndSize(35);
	...
	function BulwarkReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingBulwark");
		local self = p:GetNWEntity("Bulwark");
	...
	hook.Add("HUDPaint", "BulwarkReticle", BulwarkReticle)

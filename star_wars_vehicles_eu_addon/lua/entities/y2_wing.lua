...
ENT.EntModel = "models/sweaw/ships/rep_y2-wing.mdl" --The oath to the model you want to use.
ENT.Vehicle = "y2_wing" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("y2_wing"); -- This should be the same name as the file
...
function ENT:Effects()
    ...
    hook.Add("CalcView", "y2_wingview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function y2_wingReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingy2_wing");
		local self = p:GetNWEntity("y2_wing");
    ...
    hook.Add("HUDPaint", "y2_wingReticle", y2_wingReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

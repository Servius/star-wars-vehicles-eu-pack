...
ENT.EntModel = "models/ehawk/ehawk1.mdl" --The oath to the model you want to use.
ENT.Vehicle = "EbonHawk2" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("ebonhawk2"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 40 + self:GetRight() * 400 + self:GetUp() * 40,
        Left = self:GetPos() + self:GetForward() * 40 + self:GetRight() * -400 + self:GetUp() * 40,
		TopRight = self:GetPos() + self:GetForward() * 40 + self:GetRight() * 400 + self:GetUp() * 70,
        TopLeft = self:GetPos() + self:GetForward() * 40 + self:GetRight() * -400 + self:GetUp() * 70,
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-550+self:GetUp()*123+self:GetRight()*-105,
		self:GetPos()+self:GetForward()*-550+self:GetUp()*123+self:GetRight()*105,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("EbonHawk2", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,850,350,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "EbonHawk2view", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function EbonHawk2Reticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingEbonHawk2");
		local self = p:GetNWEntity("EbonHawk2");
    ...
    hook.Add("HUDPaint", "EbonHawk2Reticle", EbonHawk2Reticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.EntModel = "models/sfp_tiebooster/sfp_tiebooster.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tiebooster" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tiebooster"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 125 + self:GetRight() * 20 + self:GetUp() * 140,
        Left = self:GetPos() + self:GetForward() * 125 + self:GetRight() * -20 + self:GetUp() * 140,
    }
    self.WeaponsTable = {}; -- IGNORE. Needed to give players their weapons back
    self.BoostSpeed = 3200; -- The speed we go when holding SHIFT
    self.ForwardSpeed = 2300; -- The forward speed 
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-200+self:GetUp()*178
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tiebooster", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,400,300,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_tieboosterview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tieboosterReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tiebooster");
		local self = p:GetNWEntity("sfp_tiebooster");
    ...
    hook.Add("HUDPaint", "sfp_tieboosterReticle", sfp_tieboosterReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

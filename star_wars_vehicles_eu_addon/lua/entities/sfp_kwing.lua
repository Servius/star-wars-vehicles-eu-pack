...
ENT.EntModel = "models/sfp_kwing/sfp_kwing.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_kwing" --The internal name for the ship. It cannot be the same as a different ship.
ENT.StartHealth = 3200; --How much health they should have.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_kwing"); -- This should be the same name as the file
	local spawn_height = 60; -- How high above the ground the vehicle spawns. Change if it's spawning too high, or spawning in the ground.
...
function ENT:Initialize()
	...
	self.FireGroup = {"Left","Right","Left2","Right2"} -- In this example, the weapon positions set above will fire with Left and TopLeft at the same time. And Right and TopRight at the same time.
...
function ENT:Think()
    ...
    if(self.Inflight) then
        //self.AccelSpeed = math.Approach(self.AccelSpeed,-5,1);
...
function ENT:Effects()
    ...
    hook.Add("CalcView", "sfp_kwingview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_kwingReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_kwing");
		local self = p:GetNWEntity("sfp_kwing");
    ...
    hook.Add("HUDPaint", "sfp_kwingReticle", sfp_kwingReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

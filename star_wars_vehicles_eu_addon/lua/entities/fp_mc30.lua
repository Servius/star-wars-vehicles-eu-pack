...
function ENT:Initialize()
	...
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*-100+self:GetForward()*-1600+self:GetRight()*0, self:GetAngles()+Angle(0,90,0)},
		{self:GetPos()+self:GetUp()*-0+self:GetForward()*1200+self:GetRight()*0, self:GetAngles()+Angle(0,270,0)},
	}
	self.GunnerSeats = {};
	...
	self.LeftWeaponLocations = {
		self:GetPos()+self:GetUp()*-180+self:GetForward()*-1800+self:GetRight()*-15,
		self:GetPos()+self:GetUp()*-180+self:GetForward()*-1800+self:GetRight()*15,
		self:GetPos()+self:GetUp()*-190+self:GetForward()*-1800
	}
	
	self.RightWeaponLocations = {
		self:GetPos()+self:GetUp()*-50+self:GetForward()*1400+self:GetRight()*-15,
		self:GetPos()+self:GetUp()*-50+self:GetForward()*1400+self:GetRight()*15,
		self:GetPos()+self:GetUp()*-60+self:GetForward()*1400
	}
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		local pos;
		if(fire == 1) then
			pos = self:GetPos()+self:GetUp()*-145+self:GetForward()*-2300+self:GetRight()*-65, self:GetAngles()+Angle(0,180,0);
...
hook.Add("PlayerLeaveVehicle", "mc30SeatExit", function(p,v)
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*300+self:GetUp()*-10+self:GetRight()*150,
				self:GetPos()+self:GetForward()*300+self:GetUp()*-10+self:GetRight()*-150,
				self:GetPos()+self:GetForward()*2170+self:GetUp()*120
			}
			self.EnginePos2 = {
				self:GetPos()+self:GetForward()*200+self:GetUp()*10+self:GetRight()*255,
				self:GetPos()+self:GetForward()*200+self:GetUp()*10+self:GetRight()*-255,
			}
			self:Effects();
		end
	...
	function mc30Reticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingmc30");
		local self = p:GetNWEntity("mc30");
	...
	hook.Add("HUDPaint", "mc30Reticle", mc30Reticle)

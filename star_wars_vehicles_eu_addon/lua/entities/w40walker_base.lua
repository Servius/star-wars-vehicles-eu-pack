...
ENT.PrintName = "Warhammer Walker Base"
ENT.Author = "Liam0102"
ENT.Category = "Star Wars"
ENT.AutomaticFrameAdvance = true
ENT.Spawnable = false;
ENT.AdminSpawnable = false;
ENT.IsW40WALKERVehicle = true;
...
function IsW40WALKERSpeeder(class)
...
if SERVER then
function W40WALKERCreateBulletStructure(dmg,color,nosplashdamage)
	...
	local bullet = {
		...
		Callback = function(p,tr,damage)
			...
			if(IsValid(self) and self != tr.Entity) then
				...
				if(tr.Entity.IsW40WALKERVehicle and tr.Entity.Inflight) then
				...
				if(color == "blue" and !noion) then
					if(tr.Entity.IsW40WALKERVehicle) then
...
function ENT:Enter(p,driver)
	...
	p:SetNWInt("W40WALKER_Speeder_MaxSpeed",self.BoostSpeed)
...
hook.Add("PlayerEnteredVehicle","W40WALKERSpeederUse", function(p, e)
	if(IsValid(e) and IsValid(p) and p:IsPlayer()) then
		local speeder = e:GetParent();
		if(IsValid(speeder) and speeder.IsW40WALKERVehicle) then
		...
		if(self.Inflight and IsValid(self.Pilot)) then
			...
			*/
			...
			if(self.EngineOn) then
				...
				if(IsValid(self.Pilot)) then
					self.Pilot:SetNWInt("W40WALKER_Speeder_Speed",self.Accel.FWD);
				end
				
				if(self.Pilot:KeyDown(IN_MOVERIGHT)) then
					self.YawAccel = -20 - (self.Accel.FWD/100);
...
function W40WALKERSpeederStandbyPickup(p,e)
	if(IsValid(e) and e.IsW40WALKERVehicle) then
		e.ShouldStandby = false;
	end
end
hook.Add( "PhysgunPickup", "W40WALKERSpeederStandbyPickup", W40WALKERSpeederStandbyPickup )

function W40WALKERSpeederStandbyDrop(p,e)
	if(IsValid(e) and e.IsW40WALKERVehicle) then
		e.ShouldStandby = true;
	end
end
hook.Add("PhysgunDrop", "W40WALKERSpeederStandbyDrop", W40WALKERSpeederStandbyDrop);
...
if CLIENT then
	...
	function ENT:Think()
		...
		if self:GetNWInt("Speed") >= 5 then
			...
			Speed = p:GetNWInt("W40WALKER_Speeder_Speed") or self:GetNWInt("Speed");
			MaxSpeed = p:GetNWInt("W40WALKER_Speeder_MaxSpeed") or self:GetNWInt("MaxSpeed");
	...
	function W40WALKER_Speeder_DrawHull(StartHealth)
	...
	function W40WALKER_Speeder_DrawSpeedometer()
	...
	function W40WALKER_Speeder_Reticles(self,WeaponsPos) 
		local p = LocalPlayer();
		...
		for i=1,table.Count(WeaponsPos) do
			...
			if(IsValid(tr.Entity)) then
				if(IsW40WALKERSpeeder(tr.Entity:GetClass()) and tr.Entity != self) then

...
ENT.EntModel = "models/sfp_bwing/sfp_bwing.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_bwing" --The internal name for the ship. It cannot be the same as a different ship.
ENT.StartHealth = 2100; --How much health they should have.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_bwing"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 1167 + self:GetRight() * 235 + self:GetUp() * 0,
        Left = self:GetPos() + self:GetForward() * 1167 + self:GetRight() * -235 + self:GetUp() * 0,
        Bottom = self:GetPos() + self:GetForward() * 1145 + self:GetRight() * 0 + self:GetUp() * -520,
        Bottom2 = self:GetPos() + self:GetForward() * 1145 + self:GetRight() * 0 + self:GetUp() * -520,
    }
	...
	self.FireGroup = {"Left","Right","Bottom","Bottom2"} -- In this example, the weapon positions set above will fire with Left and TopLeft at the same time. And Right and TopRight at the same time.
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		local pos;
		if(fire == 1) then
			pos = self:GetPos()+self:GetUp()*1025+self:GetForward()*60+self:GetRight()*-50;
			self.NextUse.Torpedos = CurTime()-0.1;
		elseif(fire == 2) then
			pos = self:GetPos()+self:GetUp()*1025+self:GetForward()*60+self:GetRight()*50;
			self.NextUse.Torpedos = CurTime()+0.25;
		elseif(fire == 3) then
			pos = self:GetPos()+self:GetUp()*1080+self:GetForward()*60+self:GetRight()*-50;
			self.NextUse.Torpedos = CurTime()-0.1;
		elseif(fire == 4) then
			pos = self:GetPos()+self:GetUp()*1080+self:GetForward()*60+self:GetRight()*50;
		...
		self:FireTorpedo(pos,e,1500,500,Color(230,20,0,200),16);
		fire = fire + 1;
		if(fire > 4) then
			fire = 1;
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-115+self:GetUp()*37+self:GetRight()*-25,
		self:GetPos()+self:GetForward()*-115+self:GetUp()*37+self:GetRight()*25,
		self:GetPos()+self:GetForward()*-115+self:GetUp()*87+self:GetRight()*-25,
		self:GetPos()+self:GetForward()*-115+self:GetUp()*87+self:GetRight()*25,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_bwing", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,500,1200,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_bwingview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_bwingReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_bwing");
		local self = p:GetNWEntity("sfp_bwing");
    ...
    hook.Add("HUDPaint", "sfp_bwingReticle", sfp_bwingReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.EntModel = "models/fp_refinery/fp_refinery.mdl"
ENT.Vehicle = "fp_refinery"
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("fp_refinery");
...
function ENT:Initialize()
	...
	self.WeaponLocations = {
		Left = self:GetPos()+self:GetForward()*100+self:GetUp()*70+self:GetRight()*-170,
		Right = self:GetPos()+self:GetForward()*100+self:GetUp()*70+self:GetRight()*170,
	}
	self.WeaponsTable = {};
	self.BoostSpeed = 650;
	...
	self.HasWings = false;
	self.CanShoot = false;
	self.Bullet = CreateBulletStructure(75,"yellow",false);
	self.FireDelay = 0.75;
	self.HasLightspeed = true;
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*250+self:GetForward()*-500+self:GetRight()*600, self:GetAngles()+Angle(0,180,0)},
		{self:GetPos()+self:GetUp()*250+self:GetForward()*-500+self:GetRight()*-600, self:GetAngles()},
	}
	self.GunnerSeats = {};
	...
	self.LeftWeaponLocations = {
		self:GetPos()+self:GetUp()*-30+self:GetForward()*200+self:GetRight()*1050,
		self:GetPos()+self:GetUp()*-30+self:GetForward()*230+self:GetRight()*1050,
		
		self:GetPos()+self:GetUp()*-30+self:GetForward()*-140+self:GetRight()*1050,
		self:GetPos()+self:GetUp()*-30+self:GetForward()*-170+self:GetRight()*1050,
		
		self:GetPos()+self:GetUp()*-30+self:GetForward()*-500+self:GetRight()*1100,
		self:GetPos()+self:GetUp()*-30+self:GetForward()*-530+self:GetRight()*1100,
		
		self:GetPos()+self:GetUp()*-30+self:GetForward()*-950+self:GetRight()*1100,
		self:GetPos()+self:GetUp()*-30+self:GetForward()*-980+self:GetRight()*1100,		
	}
...
hook.Add("PlayerLeaveVehicle", "RefinerySeatExit", function(p,v)
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*785+self:GetUp()*-240+self:GetRight()*-300,
				self:GetPos()+self:GetForward()*785+self:GetUp()*-240+self:GetRight()*-560,
				self:GetPos()+self:GetForward()*785+self:GetUp()*-240+self:GetRight()*-805,
				
				self:GetPos()+self:GetForward()*785+self:GetUp()*-240+self:GetRight()*340,
				self:GetPos()+self:GetForward()*785+self:GetUp()*-240+self:GetRight()*600,
				self:GetPos()+self:GetForward()*785+self:GetUp()*-240+self:GetRight()*850,
			}
			self:Effects();
		end
	end	
	...
	function RefineryReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingRefinery");
		local self = p:GetNWEntity("Refinery");
	...
	hook.Add("HUDPaint", "RefineryReticle", RefineryReticle)

...
ENT.EntModel = "models/swbf3/vehicles/imp_tie-interceptor.mdl" --The oath to the model you want to use.
ENT.Vehicle = "StarHunter" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("starhunter"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 235 + self:GetRight() * 190 + self:GetUp() * 125,
        Left = self:GetPos() + self:GetForward() * 235 + self:GetRight() * -190 + self:GetUp() * 125,
		TopRight = self:GetPos() + self:GetForward() * 235 + self:GetRight() * 190 + self:GetUp() * 225,
        TopLeft = self:GetPos() + self:GetForward() * 235 + self:GetRight() * -190 + self:GetUp() * 225,
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-165+self:GetUp()*175+self:GetRight()*-0,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("StarHunter", NULL)
    ...
    hook.Add("CalcView", "StarHunterview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function StarHunterReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingStarHunter");
		local self = p:GetNWEntity("StarHunter");
    ...
    hook.Add("HUDPaint", "StarHunterReticle", StarHunterReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.EntModel = "models/sfp_tiereaper/sfp_tiereaper.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tiereaper" --The internal name for the ship. It cannot be the same as a different ship.
ENT.StartHealth = 2500; --How much health they should have.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tiereaper"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        TopRight = self:GetPos() + self:GetForward() * 450 + self:GetRight() * 150 + self:GetUp() * 195,
        TopLeft = self:GetPos() + self:GetForward() * 450 + self:GetRight() * -150 + self:GetUp() * 195,
        Right = self:GetPos() + self:GetForward() * 450 + self:GetRight() * 150 + self:GetUp() * 70,
        Left = self:GetPos() + self:GetForward() * 450 + self:GetRight() * -150 + self:GetUp() * 70,
    }
	...
	self.ExitModifier = {x=0,y=-350,z=10}
    ...
    self.Bullet = CreateBulletStructure(80,"green"); -- The first number is bullet damage, the second colour. green and red are the only options. (Set to blue for ion shot, the damage will be halved but ships will be disabled after consecutive hits). The final one is for splash damage. Set to true if you don't want splashdamage.
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		local pos;
		if(fire == 1) then
			pos = self:GetPos()+self:GetUp()*100+self:GetForward()*100+self:GetRight()*-15;
		...
		self:FireTorpedo(pos,e,1500,500,Color(130,180,255,200),10);
		fire = fire + 1;
		if(fire > 2) then
			fire = 1;
...
if CLIENT then
    ...
    ENT.Sounds={
        ...
        Engine=Sound("vehicles/tie/tie_engine.wav"), -- This is the flight sound. These can get complicated, so I'd use the ones I've already put in the addon
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-50+self:GetUp()*130
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tiereaper", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,300,300,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_tiereaperview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tiereaperReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tiereaper");
		local self = p:GetNWEntity("sfp_tiereaper");
		if(Flying and IsValid(self)) then
			SW_HUD_DrawHull(2500); -- Replace 1000 with the starthealth at the top
    ...
    hook.Add("HUDPaint", "sfp_tiereaperReticle", sfp_tiereaperReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

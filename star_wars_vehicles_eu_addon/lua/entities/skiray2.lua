...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("skiray2"); // This should be the same name as the file
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-235+self:GetUp()*70+self:GetRight()*-0,
		self:GetPos()+self:GetForward()*-150+self:GetUp()*70+self:GetRight()*-95,
		self:GetPos()+self:GetForward()*-150+self:GetUp()*70+self:GetRight()*95,
	}
	
	for k,v in pairs(self.EnginePos) do
	
		local heat = self.FXEmitter:Add("sprites/orangecore1",v) // This is where you add the effect. The ones I use are either the current or "sprites/bluecore"
		...
		heat:SetStartSize(15) // Start size. Just play around to find the right size.
    ...
    hook.Add("CalcView", "Skiray2view", CalcView) // This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function Skiray2Reticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingSkiray2");
		local self = p:GetNWEntity("Skiray2");
    ...
    hook.Add("HUDPaint", "Skiray2Reticle", Skiray2Reticle) // Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

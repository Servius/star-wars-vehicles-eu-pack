...
ENT.EntModel = "models/sfp_tiescout/sfp_tiescout.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tiescout" --The internal name for the ship. It cannot be the same as a different ship.
ENT.StartHealth = 1000; --How much health they should have.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tiescout"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.Bullet = CreateBulletStructure(40,"green",true); -- The first number is bullet damage, the second colour. green and red are the only options. (Set to blue for ion shot, the damage will be halved but ships will be disabled after consecutive hits). The final one is for splash damage. Set to true if you don't want splashdamage.
...
if CLIENT then
    ...
    ENT.Sounds={
        ...
        Engine=Sound("vehicles/tie/tie_new.wav"), -- This is the flight sound. These can get complicated, so I'd use the ones I've already put in the addon
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-150+self:GetUp()*130,
		self:GetPos()+self:GetForward()*-150+self:GetUp()*70
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetColor(130,150,255) -- Set the colour in RGB. This is more of an overlay colour effect and doesn't change the material source.
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tiescout", NULL)
    ...
    hook.Add("CalcView", "sfp_tiescoutview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tiescoutReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tiescout");
		local self = p:GetNWEntity("sfp_tiescout");
    ...
    hook.Add("HUDPaint", "sfp_tiescoutReticle", sfp_tiescoutReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.EntModel = "models/sfp_tieinterceptor/sfp_tieinterceptor.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tieinterceptor" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tieinterceptor"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        TopRight = self:GetPos() + self:GetForward() * 250 + self:GetRight() * 130 + self:GetUp() * 150,
        TopLeft = self:GetPos() + self:GetForward() * 250 + self:GetRight() * -130 + self:GetUp() * 150,
        Right = self:GetPos() + self:GetForward() * 250 + self:GetRight() * 130 + self:GetUp() * 75,
        Left = self:GetPos() + self:GetForward() * 250 + self:GetRight() * -130 + self:GetUp() * 75,
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-35+self:GetUp()*110+self:GetRight()*-34,
		self:GetPos()+self:GetForward()*-35+self:GetUp()*110+self:GetRight()*34,
	}
    ...
    hook.Add("CalcView", "havoc_mercview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function havoc_mercReticle()
    ...
    hook.Add("HUDPaint", "havoc_mercReticle", havoc_mercReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.PrintName = "Supremacy Fighter-2";
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("supremacyfighter2"); // This should be the same name as the file
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-168+self:GetUp()*232+self:GetRight()*-45,
		self:GetPos()+self:GetForward()*-168+self:GetUp()*232+self:GetRight()*45,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("SupremacyFighter2", NULL)
    ...
    hook.Add("CalcView", "SupremacyFighter2view", CalcView) // This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function SupremacyFighter2Reticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingSupremacyFighter2");
		local self = p:GetNWEntity("SupremacyFighter2");
    ...
    hook.Add("HUDPaint", "SupremacyFighter2Reticle", SupremacyFighter2Reticle) // Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

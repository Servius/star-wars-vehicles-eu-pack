...
ENT.EntModel = "models/sfp_tiedroid/sfp_tiedroid.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tiedroid" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tiedroid"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 140 + self:GetRight() * 10 + self:GetUp() * 40,
        Left = self:GetPos() + self:GetForward() * 140 + self:GetRight() * -10 + self:GetUp() * 40,
    }
    ...
    self.BaseClass.Initialize(self); -- Ig
end
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-105+self:GetUp()*75+self:GetRight()*-33,
		self:GetPos()+self:GetForward()*-105+self:GetUp()*75+self:GetRight()*33,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetStartSize(3) -- Start size. Just play around to find the right size.
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tiedroid", NULL)
    ...
    hook.Add("CalcView", "sfp_tiedroidview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tiedroidReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tiedroid");
		local self = p:GetNWEntity("sfp_tiedroid");
    ...
    hook.Add("HUDPaint", "sfp_tiedroidReticle", sfp_tiedroidReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

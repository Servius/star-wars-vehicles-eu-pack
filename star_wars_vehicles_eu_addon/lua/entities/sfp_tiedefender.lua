...
ENT.EntModel = "models/sfp_tiedefender/sfp_tiedefender.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tiedefender" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tiedefender"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        FarRight = self:GetPos() + self:GetForward() * 200 + self:GetRight() * 80 + self:GetUp() * 100,
        FarLeft = self:GetPos() + self:GetForward() * 200 + self:GetRight() * -80 + self:GetUp() * 90,
        Right = self:GetPos() + self:GetForward() * 200 + self:GetRight() * 120 + self:GetUp() * 170,
        Left = self:GetPos() + self:GetForward() * 200 + self:GetRight() * -120 + self:GetUp() * 170,
    }
	...
	self.ExitModifier = {x=0,y=225,z=60}
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		local pos;
		if(fire == 1) then
			pos = self:GetPos()+self:GetUp()*145+self:GetForward()*-30+self:GetRight()*-15;
		...
		self:FireTorpedo(pos,e,1500,500,Color(225,85,15,100),10);
		fire = fire + 1;
		if(fire > 2) then
			fire = 1;
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-100+self:GetUp()*185
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tiedefender", NULL)
    ...
    hook.Add("CalcView", "sfp_tiedefenderview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tiedefenderReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tiedefender");
		local self = p:GetNWEntity("sfp_tiedefender");
    ...
    hook.Add("HUDPaint", "sfp_tiedefenderReticle", sfp_tiedefenderReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

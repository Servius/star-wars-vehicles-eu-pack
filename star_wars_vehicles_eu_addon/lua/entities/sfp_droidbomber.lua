...
ENT.EntModel = "models/sfp_droidbomber/sfp_droidbomber.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_droidbomber" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_droidbomber"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        TopRight = self:GetPos() + self:GetForward() * 64 + self:GetRight() * 156 + self:GetUp() * 54,
        TopLeft = self:GetPos() + self:GetForward() * 64 + self:GetRight() * -156 + self:GetUp() * 54,
        Right = self:GetPos() + self:GetForward() * 64 + self:GetRight() * 156 + self:GetUp() * 42,
        Left = self:GetPos() + self:GetForward() * 64 + self:GetRight() * -156 + self:GetUp() * 42,
    }
	...
	self.ExitModifier = {x=125,y=225,z=50}
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		local pos;
		if(fire == 1) then
			pos = self:GetPos()+self:GetUp()*52+self:GetForward()*120+self:GetRight()*50;
		...
		self:FireTorpedo(pos,e,1500,500,Color(0,0,230,200),10);
		fire = fire + 1;
		if(fire > 2) then
			fire = 1;
...
if CLIENT then
    ...
    ENT.Sounds={
        ...
        Engine=Sound("vehicles/droid/droid_fly.wav"), -- This is the flight sound. These can get complicated, so I'd use the ones I've already put in the addon
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-165+self:GetUp()*48+self:GetRight()*-76,
		self:GetPos()+self:GetForward()*-165+self:GetUp()*48+self:GetRight()*76,
		self:GetPos()+self:GetForward()*-180+self:GetUp()*48+self:GetRight()*-46,
		self:GetPos()+self:GetForward()*-180+self:GetUp()*48+self:GetRight()*46,
		self:GetPos()+self:GetForward()*-185+self:GetUp()*48+self:GetRight()*-16,
		self:GetPos()+self:GetForward()*-185+self:GetUp()*48+self:GetRight()*16,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetColor(180,0,90) -- Set the colour in RGB. This is more of an overlay colour effect and doesn't change the material source.
		...
		dynlight.R = 180; -- Colour R
		dynlight.G = 0; -- Colour G
		dynlight.B = 90; -- Colour B
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_droidbomber", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,400,150,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_droidbomberview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_droidbomberReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_droidbomber");
		local self = p:GetNWEntity("sfp_droidbomber");
    ...
    hook.Add("HUDPaint", "sfp_droidbomberReticle", sfp_droidbomberReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.NextUse = {Use = CurTime(),Fire = CurTime(),Laser = CurTime()};
ENT.FlamethrowerSound = Sound("ambient/fire/ignite.wav");
ENT.LaserSound = Sound("w40ksentinel/autocannon.wav");


AddCSLuaFile();
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("w40ksentinel2");
...
function ENT:Think()
    ...
    if(self.Inflight) then
	    ...
	    local aimvector = self.Pilot:GetAimVector():Angle():Forward()*10000
	    local saveangle = self.Pilot:GetAimVector():Angle()
	    local weaponangle = self:WorldToLocalAngles( saveangle )
	    local aim = weaponangle;	
		local laserpos,laserang = self:GetBonePosition(self:LookupBone("Cannon")) 
		...
		if(self.Pilot:KeyDown(IN_ATTACK)) then
			self:FireLaser(laserpos+self:GetRight()*0+self:GetForward()*0,false,1000,aimvector);
		end
			lastZ = aim.z;
			self:NextThink(CurTime());
			return true;
...
function ENT:FireLaser(pos,gravity,vel,ang)
    if(self.NextUse.Laser< CurTime()) then
        local e = ents.Create("sentinellaser");
        e:SetPos(pos);
        e:Spawn();
        e:Activate();
        e:Prepare(self,Sound("w40ksentinel/autocannon.wav"),gravity,vel,ang);
        e:SetColor(Color(255,255,255,1));
		
		self:EmitSound(self.LaserSound, 120, math.random(110,160));
        self.NextUse.Laser = CurTime() + 0.3;
    end
end
...
if CLIENT then
	...
	function CalcView()
		...
		local p = LocalPlayer();
		local self = p:GetNWEntity("W40KSentinel2", NULL)
	...
	hook.Add("CalcView", "W40KSentinel2View", CalcView)
	
	function W40KSentinel2Reticle()
	
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingW40KSentinel2");
		local self = p:GetNWEntity("W40KSentinel2");
		if(Flying and IsValid(self)) then		
			...
			tr = util.TraceLine({
				start = headPoss,
				endpos = headPoss + p:GetAimVector():Angle():Forward()*10000,
				filter = {self,p},
			})
	...
	hook.Add("HUDPaint", "W40KSentinel2Reticle", W40KSentinel2Reticle)

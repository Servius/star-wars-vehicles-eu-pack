...
ENT.EntModel = "models/sfp_planetary/sfp_planetary.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_planetary" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_planetary"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 510 + self:GetRight() * 30 + self:GetUp() * 190,
        Left = self:GetPos() + self:GetForward() * 510 + self:GetRight() * -30 + self:GetUp() * 190,
    }
    self.WeaponsTable = {}; -- IGNORE. Needed to give players their weapons back
    self.BoostSpeed = 4500; -- The speed we go when holding SHIFT
    self.ForwardSpeed = 750; -- The forward speed 
    self.UpSpeed = 500; -- Up/Down Speed
    self.AccelSpeed = 35; -- How fast we get to our previously set speeds
	...
	self.ExitModifier = {x=225,y=250,z=10}
    ...
    self.Bullet = CreateBulletStructure(60,"red"); -- The first number is bullet damage, the second colour. green and red are the only options. (Set to blue for ion shot, the damage will be halved but ships will be disabled after consecutive hits). The final one is for splash damage. Set to true if you don't want splashdamage.
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-340+self:GetUp()*120+self:GetRight()*-50,
		self:GetPos()+self:GetForward()*-340+self:GetUp()*120+self:GetRight()*50,
		self:GetPos()+self:GetForward()*-340+self:GetUp()*220+self:GetRight()*-50,
		self:GetPos()+self:GetForward()*-340+self:GetUp()*220+self:GetRight()*50,
	}
	
	self.EnginePos2 = {
		self:GetPos()+self:GetForward()*-350+self:GetUp()*160+self:GetRight()*-500,
		self:GetPos()+self:GetForward()*-350+self:GetUp()*160+self:GetRight()*500,
		self:GetPos()+self:GetForward()*-350+self:GetUp()*160+self:GetRight()*-470,
		self:GetPos()+self:GetForward()*-350+self:GetUp()*160+self:GetRight()*470,
		self:GetPos()+self:GetForward()*-180+self:GetUp()*50+self:GetRight()*-15,
		self:GetPos()+self:GetForward()*-180+self:GetUp()*50+self:GetRight()*15,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetStartSize(38) -- Start size. Just play around to find the right size.
	...
	for k,v in pairs(self.EnginePos2) do
		...
		red:SetEndSize(8) -- End size
		red:SetRoll(roll) -- They see me rollin. (They hatin')
		red:SetColor(255,70,0) -- Set the colour in RGB. This is more of an overlay colour effect and doesn't change the material source.
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_planetary", NULL)
    ...
    hook.Add("CalcView", "sfp_planetaryview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_planetaryReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_planetary");
		local self = p:GetNWEntity("sfp_planetary");
    ...
    hook.Add("HUDPaint", "sfp_planetaryReticle", sfp_planetaryReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

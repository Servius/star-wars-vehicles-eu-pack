...
ENT.EntModel = "models/sfp_eta5/sfp_eta5.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_eta5" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_eta5"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 300 + self:GetRight() * 25 + self:GetUp() * 160,
        Left = self:GetPos() + self:GetForward() * 300 + self:GetRight() * -25 + self:GetUp() * 160,
    }
    ...
    self.Bullet = CreateBulletStructure(70,"green"); -- The first number is bullet damage, the second colour. green and red are the only options. (Set to blue for ion shot, the damage will be halved but ships will be disabled after consecutive hits). The final one is for splash damage. Set to true if you don't want splashdamage.
...
if CLIENT then
    ...
    ENT.Sounds={
        ...
        Engine=Sound("vehicles/eta/eta_fly.wav"), -- This is the flight sound. These can get complicated, so I'd use the ones I've already put in the addon
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-280+self:GetUp()*188+self:GetRight()*-22,
		self:GetPos()+self:GetForward()*-280+self:GetUp()*188+self:GetRight()*22,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetDieTime(0.06) --How quick the particle dies. Make it larger if you want the effect to hang around
    ...
    hook.Add("CalcView", "sfp_eta5view", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_eta5Reticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_eta5");
		local self = p:GetNWEntity("sfp_eta5");
    ...
    hook.Add("HUDPaint", "sfp_eta5Reticle", sfp_eta5Reticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

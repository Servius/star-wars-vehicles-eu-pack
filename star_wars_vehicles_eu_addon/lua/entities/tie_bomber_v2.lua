...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("tie_bomber_v2");
...
function ENT:Initialize()
	...
	self.WeaponLocations = {
		Left = self:GetPos()+self:GetForward()*130+self:GetUp()*55+self:GetRight()*73,
		Right = self:GetPos()+self:GetForward()*130+self:GetUp()*55+self:GetRight()*43,
	}
	self.WeaponsTable = {};
	...
	if(self.Inflight) then
		if(IsValid(self.Pilot)) then
			if(self.Pilot:KeyDown(IN_ATTACK2)) then
				self:FireBlast(self:GetPos()+self:GetForward()*50+self:GetRight()*-70+self:GetUp()*75,true,1.0);
			end
		end
	end

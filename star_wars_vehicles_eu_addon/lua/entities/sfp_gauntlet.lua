...
ENT.EntModel = "models/sfp_gauntlet/sfp_gauntlet.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_gauntlet" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_gauntlet"); -- This should be the same name as the file
	local spawn_height = 29; -- How high above the ground the vehicle spawns. Change if it's spawning too high, or spawning in the ground.
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 500 + self:GetRight() * 30 + self:GetUp() * 150,
        Left = self:GetPos() + self:GetForward() * 500 + self:GetRight() * -30 + self:GetUp() * 150,
    }
	...
	self.ExitModifier = {x=125,y=225,z=10}
...
if CLIENT then
    ...
    ENT.Sounds={
        ...
        Engine=Sound("vehicles/landspeeder/t47_fly2.wav"), -- This is the flight sound. These can get complicated, so I'd use the ones I've already put in the addon
    }
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-470+self:GetUp()*170+self:GetRight()*-265,
		self:GetPos()+self:GetForward()*-470+self:GetUp()*170+self:GetRight()*265,
		self:GetPos()+self:GetForward()*-470+self:GetUp()*170+self:GetRight()*-205,
		self:GetPos()+self:GetForward()*-470+self:GetUp()*170+self:GetRight()*205,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_gauntlet", NULL)
    ...
    hook.Add("CalcView", "sfp_gauntletview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_gauntletReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_gauntlet");
		local self = p:GetNWEntity("sfp_gauntlet");
    ...
    hook.Add("HUDPaint", "sfp_gauntletReticle", sfp_gauntletReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.EntModel = "models/sfp_tieneutralizer/sfp_tieneutralizer.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tieneutralizer" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tieneutralizer"); -- This should be the same name as the file
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-180+self:GetUp()*110+self:GetRight()*-15,
		self:GetPos()+self:GetForward()*-180+self:GetUp()*110+self:GetRight()*15,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetColor(100,150,255) -- Set the colour in RGB. This is more of an overlay colour effect and doesn't change the material source.
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tieneutralizer", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,500,270,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_tieneutralizerview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tieneutralizerReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tieneutralizer");
		local self = p:GetNWEntity("sfp_tieneutralizer");
    ...
    hook.Add("HUDPaint", "sfp_tieneutralizerReticle", sfp_tieneutralizerReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

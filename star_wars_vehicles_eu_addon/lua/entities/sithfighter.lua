...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sithfighter"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 325 + self:GetRight() * 75 + self:GetUp() * 16,
        Left = self:GetPos() + self:GetForward() * 325 + self:GetRight() * -75 + self:GetUp() * 16,
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-110+self:GetUp()*40+self:GetRight()*-0,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		heat:SetStartSize(40) -- Start size. Just play around to find the right size.
    ...
    hook.Add("CalcView", "SithFighterview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function SithFighterReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingSithFighter");
		local self = p:GetNWEntity("SithFighter");
    ...
    hook.Add("HUDPaint", "SithFighterReticle", SithFighterReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

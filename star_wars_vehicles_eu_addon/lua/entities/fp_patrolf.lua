...
ENT.EntModel = "models/fp_patrolf/fp_patrolf.mdl"
ENT.Vehicle = "fp_patrolf"
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("fp_patrolf");
...
function ENT:Initialize()
	...
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*-200+self:GetForward()*150+self:GetRight()*400, self:GetAngles()+Angle(0,180,0)},
		{self:GetPos()+self:GetUp()*-200+self:GetForward()*150+self:GetRight()*-400, self:GetAngles()},
	}
	self.GunnerSeats = {};
	...
	self.LeftWeaponLocations = {
		self:GetPos()+self:GetUp()*-400+self:GetForward()*300+self:GetRight()*600,
		self:GetPos()+self:GetUp()*-400+self:GetForward()*50+self:GetRight()*600,
	}
...
hook.Add("PlayerLeaveVehicle", "PatrolFSeatExit", function(p,v)
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*475+self:GetUp()*-60+self:GetRight()*145,
				self:GetPos()+self:GetForward()*475+self:GetUp()*-60+self:GetRight()*-145,
			}
			self:Effects();
		end
	...
	function RepCruiserReticle()
		...
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingPatrolF");
		local self = p:GetNWEntity("PatrolF");
	...
	hook.Add("HUDPaint", "PatrolFReticle", PatrolFReticle)

...
ENT.EntModel = "models/sfp_kimogila/sfp_kimogila.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_kimogila" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:Initialize()
	...
	self.ExitModifier = {x=100,y=450,z=10}
...
if CLIENT then
    ...
    ENT.Sounds={
        ...
        Engine=Sound("vehicles/slave1_fly_loop.wav"), -- This is the flight sound. These can get complicated, so I'd use the ones I've already put in the addon
    }
...
function ENT:Effects()
    ...
    hook.Add("CalcView", "sfp_kimogilaview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_kimogilaReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_kimogila");
		local self = p:GetNWEntity("sfp_kimogila");
    ...
    hook.Add("HUDPaint", "sfp_kimogilaReticle", sfp_kimogilaReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("pod2ship"); // This should be the same name as the file
	local spawn_height = 35; // How high above the ground the vehicle spawns. Change if it's spawning too high, or spawning in the ground.
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-70+self:GetUp()*15+self:GetRight()*-0,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("Pod2Ship", NULL)
    ...
    hook.Add("CalcView", "Pod2Shipview", CalcView) // This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function Pod2ShipReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingPod2Ship");
		local self = p:GetNWEntity("Pod2Ship");
    ...
    hook.Add("HUDPaint", "Pod2Ship", Pod2ShipReticle) // Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.EntModel = "models/bsp_raider/bsp_raider.mdl"
ENT.Vehicle = "bsp_raider"
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("bsp_raider");
...
function ENT:Initialize()
	...
	self.HasWings = false;
	self.CanShoot = false;
	self.Bullet = CreateBulletStructure(50,"green",false);
	self.FireDelay = 0.15;
	self.HasLightspeed = true;
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*170+self:GetForward()*-400+self:GetRight()*150, self:GetAngles()+Angle(0,150,0)},
		{self:GetPos()+self:GetUp()*170+self:GetForward()*-400+self:GetRight()*-150, self:GetAngles()+Angle(0,30,0)},
	}
	self.GunnerSeats = {};
	...
	self.ExitModifier = {x=600,y=225,z=-200};
...
hook.Add("PlayerLeaveVehicle", "raiderSeatExit", function(p,v)
...
function ENT:GunnerEnter(p,right)
	...
	if(self.NextUse.Use < CurTime()) then
		if(!right) then
			if(!IsValid(self.LeftGunner)) then
				p:SetNWBool("LeftGunner_Raider",true);
				self.LeftGunner = p;
				p:EnterVehicle(self.GunnerSeats[1]);
			end
		else
			if(!IsValid(self.RightGunner)) then
				p:SetNWBool("RightGunner_Raider",true);
				self.RightGunner = p;
...
function ENT:GunnerExit(right,p)
	...
	if(!right) then
		if(IsValid(self.LeftGunner)) then
			self.LeftGunner:SetNWBool("LeftGunner_Raider",false);
			self.LeftGunner = NULL;
		end
	else
		if(IsValid(self.RightGunner)) then
			self.RightGunner:SetNWBool("RightGunner_Raider",false);
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*1375+self:GetUp()*-50
			}
			self.EnginePos2 = {
				self:GetPos()+self:GetForward()*1325+self:GetUp()*-23+self:GetRight()*345,
				self:GetPos()+self:GetForward()*1325+self:GetUp()*-23+self:GetRight()*-345,
			}
			self:Effects();
		end
	...
	function raiderReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingraider");
		local self = p:GetNWEntity("raider");
		local LeftGunner = p:GetNWBool("LeftGunner_Raider");
		local RightGunner = p:GetNWBool("RightGunner_Raider");
	...
	hook.Add("HUDPaint", "raiderReticle", raiderReticle)

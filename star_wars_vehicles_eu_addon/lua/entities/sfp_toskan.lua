...
ENT.EntModel = "models/sfp_toskan/sfp_toskan.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_toskan" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_toskan"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        TopRight = self:GetPos() + self:GetForward() * -30 + self:GetRight() * 210 + self:GetUp() * 198,
        TopLeft = self:GetPos() + self:GetForward() * -30 + self:GetRight() * -210 + self:GetUp() * 198,
        Right = self:GetPos() + self:GetForward() * -30 + self:GetRight() * 210 + self:GetUp() * 3,
        Left = self:GetPos() + self:GetForward() * -30 + self:GetRight() * -210 + self:GetUp() * 3,
    }
    ...
    self.AccelSpeed = 20; -- How fast we get to our previously set speeds
...
function ENT:ProtonTorpedos()
	...
	if(self.NextUse.Torpedos < CurTime()) then
		local pos;
		if(fire == 1) then
			pos = self:GetPos()+self:GetUp()*96+self:GetForward()*72+self:GetRight()*-9;
		...
		self:FireTorpedo(pos,e,1500,500,Color(255,60,60,255),10);
		fire = fire + 1;
		if(fire > 2) then
			fire = 1;
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-222+self:GetUp()*102+self:GetRight()*-53,
		self:GetPos()+self:GetForward()*-222+self:GetUp()*102+self:GetRight()*53,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetColor(180,150,0) -- Set the colour in RGB. This is more of an overlay colour effect and doesn't change the material source.
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_toskan", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,400,200,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "sfp_toskanview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_toskanReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_toskan");
		local self = p:GetNWEntity("sfp_toskan");
    ...
    hook.Add("HUDPaint", "sfp_toskanReticle", sfp_toskanReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

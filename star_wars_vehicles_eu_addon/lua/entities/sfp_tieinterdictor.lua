...
ENT.EntModel = "models/sfp_tieinterdictor/sfp_tieinterdictor.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_tieinterdictor" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_tieinterdictor"); -- This should be the same name as the file
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-140+self:GetUp()*120+self:GetRight()*-37,
		self:GetPos()+self:GetForward()*-140+self:GetUp()*120+self:GetRight()*37,
	}
	...
	for k,v in pairs(self.EnginePos) do
		...
		red:SetStartSize(6) -- Start size. Just play around to find the right size.
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("sfp_tieinterdictor", NULL)
    ...
    hook.Add("CalcView", "sfp_tieinterdictorview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_tieinterdictorReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_tieinterdictor");
		local self = p:GetNWEntity("sfp_tieinterdictor");
    ...
    hook.Add("HUDPaint", "sfp_tieinterdictorReticle", sfp_tieinterdictorReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.PrintName = "Marauder Artillery Frigate"
...
ENT.EntModel = "models/bsp_marauder/bsp_marauder.mdl"
ENT.Vehicle = "marauder"
...
function ENT:Initialize()
	...
	self.HasWings = false;
	self.CanShoot = false;
	self.Bullet = CreateBulletStructure(175,"red", false);
	self.FireDelay = 1.2;
	self.HasLightspeed = true;
	self.SeatPos = {
		{self:GetPos()+self:GetUp()*-250+self:GetForward()*500+self:GetRight()*1000, self:GetAngles()+Angle(0,120,0)},
		{self:GetPos()+self:GetUp()*-250+self:GetForward()*500+self:GetRight()*-1000, self:GetAngles()+Angle(0,60,0)},
	}
	self.GunnerSeats = {};
	...
	self.LeftWeaponLocations = {
		self:GetPos()+self:GetUp()*-185+self:GetForward()*-50+self:GetRight()*1200,
		self:GetPos()+self:GetUp()*-150+self:GetForward()*-75+self:GetRight()*1050,	
	}
	
	self.RightWeaponLocations = {
		self:GetPos()+self:GetUp()*-170+self:GetForward()*-50+self:GetRight()*-1200,
		self:GetPos()+self:GetUp()*-150+self:GetForward()*-75+self:GetRight()*-1050,
	}
...
function ENT:FireLeft(angPos)
	...
	if(self.NextUse.Fire < CurTime()) then
		...
		self:EmitSound(self.FireSound,100,math.random(75,100));
		self.NextUse.Fire = CurTime() + (self.FireDelay or 1.2);
	end
end
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*1625+self:GetUp()*-20+self:GetRight()*0
			}
			self.EnginePos2 = {
				self:GetPos()+self:GetForward()*1625+self:GetUp()*105+self:GetRight()*80,
				self:GetPos()+self:GetForward()*1625+self:GetUp()*105+self:GetRight()*-80,
				self:GetPos()+self:GetForward()*1625+self:GetUp()*-145+self:GetRight()*80,
				self:GetPos()+self:GetForward()*1625+self:GetUp()*-145+self:GetRight()*-80,
				self:GetPos()+self:GetForward()*1625+self:GetUp()*50+self:GetRight()*150,
				self:GetPos()+self:GetForward()*1625+self:GetUp()*-85+self:GetRight()*150,
				self:GetPos()+self:GetForward()*1625+self:GetUp()*50+self:GetRight()*-150,
				self:GetPos()+self:GetForward()*1625+self:GetUp()*-85+self:GetRight()*-150,
			}
			self:Effects();
	...
	function RepCruiserReticle()
		...
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingMarauder");
		local self = p:GetNWEntity("Marauder");
	...
	hook.Add("HUDPaint", "MarauderReticle", MarauderReticle)

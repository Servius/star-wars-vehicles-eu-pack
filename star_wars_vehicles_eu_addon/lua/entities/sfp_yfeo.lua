...
ENT.EntModel = "models/sfp_yfeo/sfp_yfeo.mdl" --The oath to the model you want to use.
ENT.Vehicle = "sfp_yfeo" --The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("sfp_yfeo"); -- This should be the same name as the file
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 275 + self:GetRight() * 15 + self:GetUp() * 20,
        Left = self:GetPos() + self:GetForward() * 275 + self:GetRight() * -15 + self:GetUp() * 20,
    }
    ...
    self.AccelSpeed = 80; -- How fast we get to our previously set speeds
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-50+self:GetUp()*60+self:GetRight()*-130,
		self:GetPos()+self:GetForward()*-50+self:GetUp()*60+self:GetRight()*130,
	}
    ...
    hook.Add("CalcView", "sfp_yfeoview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function sfp_yfeoReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingsfp_yfeo");
		local self = p:GetNWEntity("sfp_yfeo");
    ...
    hook.Add("HUDPaint", "sfp_yfeoReticle", sfp_yfeoReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

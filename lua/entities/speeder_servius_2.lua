...
ENT.Vehicle = "speeder_servius_2"; // The unique name for the speeder.
ENT.EntModel = "models/speeder_06/speeder_06.mdl"; // The path to your model
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("speeder_servius_2");
...
if CLIENT then
	...
	function CalcView()
		...
		local p = LocalPlayer();
		local self = p:GetNWEntity("speeder_servius_2", NULL)
	...
	hook.Add("CalcView", "speeder_servius_2View", CalcView)

	
	hook.Add( "ShouldDrawLocalPlayer", "speeder_servius_2DrawPlayerModel", function( p )
		local self = p:GetNWEntity("speeder_servius_2", NULL);
		local DriverSeat = p:GetNWEntity("DriverSeat",NULL);
		local PassengerSeat = p:GetNWEntity("PassengerSeat",NULL);
		if(IsValid(self)) then
	...
	function speeder_servius_2Reticle()
	
		local p = LocalPlayer();
		local Flying = p:GetNWBool("Flyingspeeder_servius_2");
		local self = p:GetNWEntity("speeder_servius_2");
	...
	hook.Add("HUDPaint", "speeder_servius_2Reticle", speeder_servius_2Reticle)

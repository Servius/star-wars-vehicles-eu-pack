...
ENT.EntModel = "models/tiehu/tiehu1.mdl" //The oath to the model you want to use.
ENT.Vehicle = "TieHunter" //The internal name for the ship. It cannot be the same as a different ship.
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("tiehunter"); // This should be the same name as the file
	local spawn_height = 55; // How high above the ground the vehicle spawns. Change if it's spawning too high, or spawning in the ground.
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 235 + self:GetRight() * 160 + self:GetUp() * 0,
        Left = self:GetPos() + self:GetForward() * 235 + self:GetRight() * -160 + self:GetUp() * 0,
		TopRight = self:GetPos() + self:GetForward() * 235 + self:GetRight() * 160 + self:GetUp() * 125,
        TopLeft = self:GetPos() + self:GetForward() * 235 + self:GetRight() * -160 + self:GetUp() * 125,
    }
	...
	self.LandOffset = Vector(0,0,5); // Change the last 0 if you're vehicle is having trouble landing properly. (Make it larger)
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-65+self:GetUp()*65+self:GetRight()*-0,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("TieHunter", NULL)
    ...
    hook.Add("CalcView", "TieHunterview", CalcView) // This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function TieHunterReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingTieHunter");
		local self = p:GetNWEntity("TieHunter");
    ...
    hook.Add("HUDPaint", "TieHunterReticle", TieHunterReticle) // Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

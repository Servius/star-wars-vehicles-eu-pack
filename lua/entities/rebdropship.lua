...
ENT.PrintName = "Rebel Drop Ship";
ENT.Author = "Liam0102, Servius";
 
// Leave the same
ENT.Category = "Star Wars: In Development"; // Techincally you could change this, but personally I'd leave it so they're all in the same place (Looks more proffesional).
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("rebdropship"); // This should be the same name as the file
...
function ENT:Initialize()
	...
	local driverAng = self:GetAngles()+Angle(0,0,0);
...
function ENT:Effects()
    ...
    hook.Add("CalcView", "RebelDropShipview", CalcView) // This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function RebelDropShipReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingRebelDropShip");
		local self = p:GetNWEntity("RebelDropShip");
    ...
    hook.Add("HUDPaint", "RebelDropShipReticle", RebelDropShipReticle) // Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

...
ENT.PrintName = "Transport Speeder";
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("transportspeeder"); // This should be the same name as the file
	local spawn_height = 25; // How high above the ground the vehicle spawns. Change if it's spawning too high, or spawning in the ground.
...
function ENT:Initialize()
	...
	self.ExitModifier = {x=150,y=100,z=100}
...
function ENT:Effects()
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("TransportSpeeder", NULL)
    ...
    hook.Add("CalcView", "TransportSpeederview", CalcView) // This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function TransportSpeederReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingTransportSpeeder");
		local self = p:GetNWEntity("TransportSpeeder");
    ...
    hook.Add("HUDPaint", "TransportSpeederReticle", TransportSpeederReticle) // Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.

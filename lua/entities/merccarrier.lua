...
ENT.PrintName = "Mercenary Carrier";
...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("merccarrier"); // This should be the same name as the file
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-460+self:GetUp()*190+self:GetRight()*-51,
		self:GetPos()+self:GetForward()*-460+self:GetUp()*190+self:GetRight()*57,
	}
    ...
    hook.Add("CalcView", "MercCarrierview", CalcView) // This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function MercCarrierReticle()
    ...
    hook.Add("HUDPaint", "MercCarrierReticle", MercCarrierReticle) // Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.
